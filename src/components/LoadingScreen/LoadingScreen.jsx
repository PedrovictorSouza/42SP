...
const loremIpsum1 = 'O Lab42 é uma iniciativa da 42 São Paulo, com apoio da Mastertech.'
...
function Act1() {
  const totalWords1 = loremIpsum1.split(' ').filter(w => w.length > 0).length
  const totalWords = totalWords1
  const maxRevealTime = 2.5
  const wordDelay = maxRevealTime / (totalWords > 1 ? totalWords - 1 : 1)

  useEffect(() => {
    const textRevealCompleteTime = (totalWords * wordDelay) * 1000
  ...
  return (
    <div className="loading-screen-container">
      {showLoadingIndicator && (
        <div className="loading-indicator-wrapper" style={{ display: 'none' }}>
